...
*/int pvs(int alpha, int beta, int depth, MOVE * pBestMove, bool nulmove)
{
    ...
    //pas en échec
    //profondeur >=2
    if((!echec) && (ok_pour_nul_move()) && (depth >= 2) && (nulmove) && (!pvnode))
    {
        jouer_coup_nul();
        value = -pvs(-beta, -(beta-1), depth - NULL_DEPTH, &tmpMove, NO_NULL);
        dejouer_coup_nul();
        if (value >= beta)
            return beta;
    }
    ...
    for (i = 0; i < movecnt; ++i)
    {
        ...
        if(havemove == 1 && pvnode)
            value = -pvs(-beta, -alpha, depth - reduction, &tmpMove, NO_NULL);
        else
        {
            if((depth >= START_PROF) && (!echec) && (!echec2) &&
                    (moveBuf[i].type == NORMAL) && (havemove >= START_MOVE))
            {
                reduction += 1;
                if(pvnode)
                    reduction -= 1;
                if(history[moveBuf[i].from][moveBuf[i].dest] < -500)
                    reduction += 1;
                value = -pvs(-(alpha+1), -alpha, (depth - reduction), &tmpMove, OK_NULL);
            }
            else
                value = alpha + 1;
            //research
            ...
            if (value >= beta)     //" cutoff "
            {
                ...
                if(moveBuf[i].type == NORMAL)
                {
                    ...
                    for(k=0; k<i; ++k)
                    {
                        ...
                        {
                            history[moveBuf[k].from][moveBuf[k].dest] -= (depth * depth);
